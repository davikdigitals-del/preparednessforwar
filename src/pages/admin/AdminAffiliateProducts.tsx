...
export default function AdminAffiliateProducts() {
  ...
  const handleScrapeUrl = async () => {
    ...
    try {
      ...
      console.log(`📝 Step 1: Extracting name from URL...`);
      ...
      console.log(`📛 Extracted name: "${slugName}"`);
      ...
      console.log(`🌐 Step 2: Calling Supabase scrape-product function...`);
      let name = slugName;
      let description = "";
      let image_url = "";
      let price = 0;
      ...
      try {
        ...
        if (error) {
          ...
          console.log("⚠️ Continuing with URL-extracted data only");
        } else if (!data) {
          console.warn(`⚠️ No data returned from scrape function`);
        } else if (data.blocked) {
          console.warn(`🚫 Scraping blocked by website:`, data);
          // Don't throw - continue with basic data extraction
        } else {
          ...
          if (data.images?.length > 0) {
            ...
            console.log(`🖼️ Found ${fetchedImages.length} images`);
          }
          if (data.video_url) {
          ...
          if (data.affiliate_network) {
            ...
            console.log(`🏪 Updated network: ${scraped_affiliate_network}`);
          }
        }
      } catch (scrapeError) {
        console.error("❌ Scraper error:", scrapeError);
        // Continue execution - we'll use the extracted name from URL at minimum
        console.log("⚠️ Continuing with URL-extracted data only");
      }
