...
export default function AdminCourses() {
  ...
  const bulkDelete = async () => {
    ...
    toast({ title: "Deleted", description: `${selected.length} course(s) deleted` });
    setSelected([]); fetchCourses();
  };
  const bulkPublish = async (val: boolean) => {
    if (!selected.length) return;
    await supabase.from("courses").update({ is_published: val }).in("id", selected);
