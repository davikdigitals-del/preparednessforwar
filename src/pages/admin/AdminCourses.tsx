...
export default function AdminCourses() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to delete course. Check browser console for details.",
        variant: "destructive",
      });
    }
  };
  ...
  const allSelected = filteredCourses.length > 0 && filteredCourses.every(c => selected.includes(c.id));
  const toggleAll = () => setSelected(allSelected ? [] : filteredCourses.map(c => c.id));
  const toggleOne = (id: string) => setSelected(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  const bulkDelete = async () => {
    if (!selected.length || !confirm(`Delete ${selected.length} course(s)?`)) return;
    try {
      const { error } = await supabase.from("courses").delete().in("id", selected);
      if (error) throw error;
      setSelected([]);
      fetchCourses();
      toast({ title: "Deleted", description: `${selected.length} course(s) deleted successfully` });
    } catch (error: any) {
      console.error("Bulk delete error:", error);
      toast({
        title: "Error",
        description: error.message || `Failed to delete ${selected.length} course(s)`,
        variant: "destructive"
      });
    }
  };
  const bulkPublish = async (val: boolean) => {
    ...
    try {
      const { error } = await supabase.from("courses").update({ is_published: val }).in("id", selected);
      if (error) throw error;
      setSelected([]);
      fetchCourses();
      toast({ title: val ? "Published" : "Unpublished", description: `${selected.length} course(s) updated` });
    } catch (error: any) {
      console.error("Bulk publish error:", error);
      toast({
        title: "Error",
        description: error.message || `Failed to update ${selected.length} course(s)`,
        variant: "destructive"
      });
    }
  };
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                filteredCourses.map((course) => (
                  <tr key={course.id} className={`hover:bg-gray-50 ${selected.includes(course.id) ? "bg-blue-50" : ""}`}>
                    <td className="px-4 py-4"><input type="checkbox" checked={selected.includes(course.id)} onChange={() => toggleOne(course.id)} className="rounded" /></td>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <div className="flex items-center gap-1">
                        ...
                        {(course.rating || 0).toFixed(1)} ({course.review_count || 0})
                      </div>
                    </td>
                    <td className="px-6 py-4">
